...
for game in games:
    ...
    for i in range(3):
        element = elements[i]
        if i == 0:
            element = element.replace("Button A: X+", "")
            element = element.replace(" Y+", "")
            (x, y) = map(int, element.split(","))
            a = (y,x)

        if i == 1:
            element = element.replace("Button B: X+", "")
            element = element.replace(" Y+", "")
            (x, y) = map(int, element.split(","))
            b = (y,x)

        if i == 2:
            element = element.replace("Prize: X=", "")
            element = element.replace(" Y=", "")
            (x, y) = map(int, element.split(","))
            target = (y,x)
    ...
    for i in reversed(range(101)):
        ...
        b_y = b[0] * i
        if target[1] >= b_x:
            remains = target[1] - b_x
            aTimes = 0
            if remains % a[1] == 0:
                aTimes = int(remains / a[1])
            if (b_x + (aTimes * a[1])) == target[1] and (b_y + (aTimes * a[0])) == target[0]:
                if aTimes <= 100:
                    coins.append(i + (aTimes * 3))
